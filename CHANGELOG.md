...
- Smart monitor positioning when enabling disabled monitors: reads saved position from config file first
- Overlap detection: checks if saved position would overlap with enabled monitors
- Closest non-overlapping position calculation: finds nearest spot (left/right/top/bottom) when saved position overlaps
- Config position parser in `xwlm-cfg`: extracts saved monitor positions from Hyprland and Sway config files
- `position` parameter to `WlMonitorAction::Toggle` in `wlx_monitors` library for atomic enable+position operation

### Fixed

- Monitors no longer default to position (0,0) when enabled, causing overlap with existing monitors
- Newly enabled monitors now appear immediately in TUI layout view without requiring user interaction
